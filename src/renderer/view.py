...
class CurveView(object):
    ...
    def __init__(self, width: int, height: int) -> None:
        self.controller: controller.CurveController = controller.CurveController(self)
    ...
    def callbackButton1(self, event: tkinter.Event) -> None:
        ...
        if self.controller.transform_state["active"]:
            self.controller.confirm_transform()
            return
        if not self.current_tool:
            self.selection_start = (event.x, event.y)
            self.selection_current = (event.x, event.y)
            shift_pressed = (event.state & 0x1) != 0
            self.controller.select_control((event.x, event.y), self.controller.mode, shift_pressed)
            self.update_display()
        ...
        if self.current_tool:
            ...
            if self.selection_start and self.selection_current:
                ...
                if dx > 2 or dy > 2:
                    ...
                    self.controller.select_area(rect, self.controller.mode, shift_pressed)
    ...
    def callbackB2Motion(self, event: tkinter.Event) -> None:
        if self.middle_mouse_pressed and self.last_mouse_pos:
            ...
            if is_shift: self.controller.pan_camera(dx, dy)
            elif is_ctrl: self.controller.zoom_camera(dy)
            else: self.controller.rotate_camera(dx, dy)
            self.last_mouse_pos = (event.x, event.y)
            self.update_display()

    def callbackMouseWheel(self, event: tkinter.Event) -> None:
        self.controller.zoom_camera(event.delta)
        self.update_display()
    ...
    def callbackToggleMode(self, event: tkinter.Event) -> None:
        ...
        self.controller.mode = 'edit' if self.controller.mode == 'viewer' else 'viewer'
        self.update_display()
    ...
    def callbackKeyPress(self, event: tkinter.Event) -> None:
        ...
        if ctrl_pressed and key == 'z':
            if not self.controller.transform_state["active"] and self.controller.get_undo_count() > 0:
                self.controller.perform_undo(); self.show_notification("Undo")
            return
        if ctrl_pressed and key == 'y':
            if not self.controller.transform_state["active"] and self.controller.get_redo_count() > 0:
                self.controller.perform_redo(); self.show_notification("Redo")
            return
        if key == 'g':
            if self.controller.transform_state["active"] and self.controller.transform_state["mode"] == 'grab':
                self.controller.cancel_transform()
            else:
                if self.controller.transform_state["active"]: self.controller.cancel_transform()
                self.controller.start_transform_mode('grab', (event.x, event.y))
        elif key == 'r':
            if self.controller.transform_state["active"] and self.controller.transform_state["mode"] == 'rotate':
                self.controller.cancel_transform()
            else:
                if self.controller.transform_state["active"]: self.controller.cancel_transform()
                self.controller.start_transform_mode('rotate', (event.x, event.y))
        elif key in ['x', 'y', 'z']:
            shift_pressed = (state & 0x1) != 0 
            self.controller.toggle_axis_constraint(key, shift_pressed)
        elif key == 'return': 
            if self.controller.transform_state["active"]: self.controller.confirm_transform()
            else: self.controller.ui_mode_apply(self.width, self.height); self.update_display()
        elif key == 'escape': self.controller.cancel_transform()
        elif key == 'up' or raw_key == 'Up':
            self.controller.ui_mode_cycle(-1); self.update_display()
        elif key == 'down' or raw_key == 'Down':
            self.controller.ui_mode_cycle(1); self.update_display()
            
    def callbackMotion(self, event: tkinter.Event) -> None:
        if self.controller.transform_state["active"]: self.controller.update_transform(event.x, event.y); return
        if self.selection_start: self.selection_current = (event.x, event.y); self.update_display()

    def on_new(self) -> None:
        if self.controller.transform_state["active"]: self.controller.cancel_transform()
        self.controller = controller.CurveController(self); self.update_display()

    def on_horizontal(self) -> None:
        if self.controller.transform_state["active"]: self.controller.cancel_transform()
        self.current_tool = self.controller.new_horizontal()

    def on_vertical(self) -> None:
        if self.controller.transform_state["active"]: self.controller.cancel_transform()
        self.current_tool = self.controller.new_vertical()

    def on_left_right(self) -> None:
        if self.controller.transform_state["active"]: self.controller.cancel_transform()
        self.current_tool = self.controller.new_left_right()

    def on_midpoint(self) -> None:
        if self.controller.transform_state["active"]: self.controller.cancel_transform()
        self.current_tool = self.controller.new_midpoint_line()
    ...
    def on_set_mode(self, mode: str) -> None:
        if self.controller.transform_state["active"]: self.controller.cancel_transform()
        if self.controller.loaded_objects: self.controller.set_rendering_mode(self.width, self.height, mode); self.update_display()

    def update_display(self) -> None:
        if self.imageDraw and self.image and self.canvas:
            self.imageDraw.rectangle([0, 0, self.width, self.height], fill='lightgrey')
            self.controller.rebuild_curves(self.width, self.height)
            
            # 1. Draw Grid Lines
            for x1, y1, x2, y2, color in self.controller.grid_lines_2d:
                self.imageDraw.line([(x1, y1), (x2, y2)], fill=color)

            # 2. Draw Solid Faces
            for pts, color in self.controller.solid_faces_2d:
                self.imageDraw.polygon(pts, fill=color, outline=None)

            # 3. Draw Wireframe Lines
            for x1, y1, x2, y2, color in self.controller.wireframe_lines_2d:
                self.imageDraw.line([(x1, y1), (x2, y2)], fill=color)
            ...
            self.controller.draw(draw_control, draw_point)

            # 5. Draw Edit Mode Vertices
            if self.controller.mode == 'edit':
                for obj_idx, v_idx in enumerate(range(len(self.controller.projected_vertices_2d))):
                    if self.controller.current_rendering_mode in ['peintre', 'zbuffer'] and v_idx not in self.controller.visible_vertices:
                        continue
                    pos = self.controller.projected_vertices_2d[v_idx]
                    is_selected = (0, v_idx) in self.controller.selected_vertices
            ...
            theme_color = 'blue' if self.controller.mode == 'edit' else 'black'
            undo_count = self.controller.get_undo_count()
            ...
            status_parts = [f"{self.controller.mode} (tab)"]
            if self.controller.transform_state["active"]:
                mode_name = self.controller.transform_state["mode"]
                status_parts.append(mode_name)
                constraint = self.controller.transform_state["constraint"]
                if constraint:
            ...
            for idx, (label, key) in enumerate(self.controller.available_modes):
                is_selected = (idx == self.controller.ui_mode_index)
                is_active = (key == self.controller.current_rendering_mode)
    ...
    def run(self) -> None:
        fenetre = tkinter.Tk(); fenetre.title("Fundamentals 3D Renderer"); fenetre.resizable(0, 0)
